...
?>
<?
$indent_id=explode(',',$_REQUEST['indent_id']);
$countTrans=1;
?>
<div id="divTransaction">
	<?
	for($j=0;$j<count($indent_id);$j++)
	{	
		$sql_trans="SELECT * FROM ms_indent_master sm,ms_indent_transaction st WHERE sm.indent_id=st.indent_id AND sm.indent_id ='".$indent_id[$j]."' and st.pend_qty<>0";
		...
		if($rc_trans>0)
		{
            ...
            <input name="order_trans_id[]" id="order_trans_id[]" type="hidden" value="" />
						...
						<table align="center" width="100%" cellpadding="1" cellspacing="1" class="border" border="0">
							<tr class="text_tr" bgcolor="<?=$tableColor?>">
								...
								<td align="center" width="11%"><b>Rec. Qty</b></td>
								<td align="center" width="11%"><b>Pend Qty</b></td>
								<td align="center" width="5%"></td>                                  
							</tr>
							<tr class="text_tr" bgcolor="<?=$tableColor?>">
								<td align="center"><?=$countTrans?>,<?=$indent_id[$j]?></td>
								...
								<td align="center">
									<input name="rec_qty[]" id="rec_qty_<?=$countTrans?>" type="text" class="get_H_18_W_60" autocomplete="off" onkeyup="return checkQuantity(event,'rec_qty_<?=$countTrans?>','pend_qty_<?=$countTrans?>',<?=$row_t['pend_qty']?>,'')" onblur="return checkQuantity(event,'rec_qty_<?=$countTrans?>','pend_qty_<?=$countTrans?>',<?=$row_t['pend_qty']?>,'')"/>
								</td>
								<td align="center">
							...
							<tr class="text_tr" bgcolor="<?=$tableColor?>">
								<td align="center">
									<input name="rate[]" id="rate[]" type="text" class="get_H_18_W_60" autocomplete="off"/>
								</td>
								<td align="center">
