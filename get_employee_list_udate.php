...
$sql = "SELECT emp_id,badli_as,date,attendance_status FROM mpc_attendence_master where date='$date' and emp_id='$id'";
...
$sql = "SELECT mpc_employee_master.last_name,mpc_employee_master.first_name,mpc_employee_master.emp_id,mpc_employee_master.ticket_no FROM mpc_employee_master where mpc_employee_master.emp_id  and mpc_employee_master.emp_id = '$id'";
...
if (mysql_num_rows($result_doc) > 0) {
    ...
    ?>
                    ...
                    if (mysql_num_rows($result_city) > 0) {
                        ?>
                                ...
                                while ($row_city = mysql_fetch_array($result_city)) {
                                    ?> 
                                        <option value="<?= $row_city['rec_id'] ?>" <? if ($row_city['rec_id'] == $badli_as) {
                        echo 'selected="selected"';
                    } ?> ><?= $row_city['designation_name'] ?></option>
                                    <?
                                }
                                ?>
                                </select>
        ...
        <!--             <input type="hidden" id="emp_id[]" name="emp_id[]" value="<?= $row_doc['emp_id'] ?>"/> 
                    ...
                    <?
                } else if (getleavecheck("$date", $row_doc['emp_id']) == "HCL") {
                    ?>
                                                                 <input type="radio" name="attendace<?= $row_doc['emp_id'] ?>" id="attendace<?= $row_doc['emp_id'] ?>" value="PL" checked="checked" readonly="readonly"/>HALF CASUAL Leave
            <?
            ...
            ?>
                    ...
                    <select name="attendace" id="attendace" onkeydown="if (event.keyCode == 13) {
                                get_frm_attendence('get_employee_attendance_list_update.php', '<?= $date ?>', 'div_employee_last', '<?= $id ?>&str7=<?= $shift ?>', document.getElementById('attendace').value, document.getElementById('designation').value, '<?= $type ?>')
                                        }">

                       	<option value="P" <? if ($attendance_status == 'P') {
                echo 'selected="selected"';
            } ?> >Present</option>
                        <option value="CL" <? if ($attendance_status == 'Cl') {
                echo 'selected="selected"';
            } ?>>Casual Leave</option>
                        <option value="HCL" <? if ($attendance_status == 'HCl') {
                echo 'selected="selected"';
            } ?>>Half Casual Leave</option>
                        <option value="PL" <? if ($attendance_status == 'Pl') {
                echo 'selected="selected"';
            } ?>>Prelilage Leave</option>
                        <option value="A" <? if ($attendance_status == 'A') {
                echo 'selected="selected"';
            } ?>>Absent</option>
                        <option value="C/OFF" target="popup" onclick="window.open('coffavail.php?emp_id=<?= $id ?>&leave_taken=<?= $attendance_status ?>', '', 'width=600,height=400')">C/OFF</option>
    ...
    ?>       
            ...
            <tr bgcolor="#F8F8F8">
                <td colspan="8" align="center">
                    <input type="button" src="images/btn_submit.png" name="btn_attend" id="btn_attend" value="Submit" onclick=" get_frm_attendence('get_employee_attendance_list_update.php', '<?= $date ?>', 'div_employee_last', '<?= $id ?>&str7=<?= $shift ?>', document.getElementById('attendace').value, document.getElementById('designation').value, '<?= $type ?>')"/>

                    <input type="button" src="images/btn_submit.png" name="btn_attends" id="btn_attends" value="Add In C/OFF" onclick=" get_frm_attendence('get_employee_attendance_list.php', '<?= $date ?>', 'div_employee_last', '<?= $id ?>&str7=<?= $shift ?>', document.getElementById('attendace').value, document.getElementById('designation').value, '<?= $type ?>')"/>
                </td>
            </tr>    	    
        </table>
    </div>
