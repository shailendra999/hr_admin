...
{
	$mode = $_GET["mode"];
	$sql_idate="select * from elec_CM where insert_date='".date('Y-m-d')."' and $PageKey='".$_GET[$PageKey]."'";
...
if(isset($_POST["btn_submit"]))
{
	...
	if($_POST['CM_date']=="")
		...
		$CM_date=getDateFormate($_POST['CM_date']);
	...
	if($department_id=='0' || $machine_id=='0' || $motor_id=='0')
	{
		...
		echo "<script>alert('Department or Machine or Compressor Not Selected.Please Select It!!!');location.href='elec_add_CM.php?Message=$Message';</script>";
	}
	else
	{
		if($PageKeyValue == "")
		{
			$tableName="elec_CM";
			$tableData=array("''","'$CM_date'","'$department_id'","'$machine_id'","'$details'","now()");
		...
		else
		{
			...
			{
				$tableName="elec_CM";
				$tableColumns=array("CM_id","CM_date","department_id","machine_id","details");
				$tableData=array("'$PageKeyValue'","'$CM_date'","'$department_id'","'$machine_id'","'$details'");
				updateDataIntoTable($tableName,$tableColumns,$tableData);
				$Message = "$PageFor Updated";
				redirect("elec_list_CM.php?Message=$Message");
			}
		}
	}
}
...
{
	$sql = "select * from elec_CM where $PageKey = '".$_GET[$PageKey]."'";
	$result = mysql_query ($sql) or die ("Invalid query : ".$sql."<br>".mysql_errno()." : ".mysql_error());
	if(mysql_num_rows($result)>0)
	{
		$row = mysql_fetch_array($result);
		$PageKeyValue = $row[$PageKey];
		$CM_date = getDateFormate($row["CM_date"]);	
		$details= $row["details"];		
...
{
	$sql="select max(CM_id) as CM_id from elec_CM";
	$res=mysql_query($sql);
                ...
                <td valign="top" style="padding-bottom:5px;" bgcolor="#EAE3E1">
                  <form name="frm_add" id="frm_add" method="post">
                    <table width="100%" cellpadding="2" cellspacing="2" align="center" border="0" class="border text_1">
                      <tr>
                      	<td align="left"><b>Compressor Maint. No.</b></td>
                        <td align="left">
                        	<input type="text" name="CM_id" id="CM_id" readonly="readonly" value="<?= $CM_id?>" />
                        </td>
                        <td align="left"><b>Date</b></td>
                        <td align="left">
                        	<input type="text" name="CM_date" id="CM_date" readonly="readonly" value="<?=$CM_date?>" />
                          <a href="javascript:void(0)" onClick="gfPop.fPopCalendar(document.frm_add.CM_date);return false;" HIDEFOCUS>
                      ...
                      <tr>
                         ...
                         <td align="left" colspan="3">
                         	<textarea id="details" name="details" rows="3" cols="35"><?=$details?></textarea>
                         </td>
                      </tr>
                      <tr>
