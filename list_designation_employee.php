...
if (isset($_POST["btn_del"])) {
    ...
    $sql = "delete from mpc_designation_employee where rec_id = '" . $rec_id . "'";
...
if (isset($_POST['emp_category'])) {
    if ($_POST['no_refresh'] == $_SESSION['no_refresh']) {
        ...
        if (mysql_num_rows($result) > 0) {
            while ($row = mysql_fetch_array($result)) {
                ...
                if (strtotime($txt_from_date) >= strtotime($row['from_date']) and strtotime($txt_from_date) <= strtotime($row['to_date'])) {
                    $sql_check = "Update " . $mysql_table_prefix . "designation_employee set	
                    ...
                    $sql = "delete from " . $mysql_table_prefix . "designation_employee where rec_id='" . $row['rec_id'] . "' and to_date!='0000-00-00'";
                    mysql_query($sql) or die("Invalid query : " . $sql . "<br>" . mysql_errno() . " : " . mysql_error());

                    if (strtotime($current_date) == strtotime($txt_to_date)) {
                        $sql = "delete from " . $mysql_table_prefix . "designation_employee where rec_id='" . $row['rec_id'] . "'";
                        mysql_query($sql) or die("Invalid query : " . $sql . "<br>" . mysql_errno() . " : " . mysql_error());
                    }
                }
            }
        ...
        $sql_check = "Update mpc_designation_employee set designation_id ='$designation',from_date 	='$txt_from_date',to_date ='$txt_to_date' where emp_id ='$id'";

        $result_check = mysql_query($sql_check) or die("Query Failed " . mysql_error());

        $sql_check1 = "Update mpc_employee_master set designation ='$designation' where ticket_no ='$id'";
        $result_check = mysql_query($sql_check1) or die("Query Failed in employee master" . mysql_error());
    ...
    <tr>
        ...
        <td style="padding-left:5px; padding-top:5px;" valign="top">
            <table width="100%" cellpadding="0" cellspacing="0" align="center" border="0">
                <tr>
                    <td align="center" class="gray_bg">&nbsp; <a href="#" style="text-decoration:none;color:gray;">Employee master-> </a>list designation</td>
                </tr>
                <tr>
            ...
            get_frm_menu('employee_list_designation.php', this.value, 'emp_list', '');
        }" value="<?= $ticket_no ?>"/>
                                            </td>
                                            <td width="24%" class="text_1" style="padding-top:5px;">
                                                <form name="frm_check" id="frm_check">
                                                    <input type="radio" id="search_by" name="search_by" value="ID" checked="checked" onclick="get_frm_menu('employee_list_designation.php', document.getElementById('emp_search').value, 'emp_list', '')" />ID
                                                    <input type="radio" id="search_by" name="search_by" value="Name" onclick="get_frm_menu('employee_list_designation.php', document.getElementById('emp_search').value, 'emp_list', '')" />Name
                                                    <input type="radio" id="search_by" name="search_by"  value="Employee" onclick="get_frm_menu('employee_list_designation.php', document.getElementById('emp_search').value, 'emp_list', '')"/>Designation Name
                            ...
                            <tr>
                                <td valign="top">
                                    ...
                                    <?php include('load/load_designation_first.php'); ?>
                                    </div>
                                </td>
                            </tr>
                        </table>
...
<div id="overlay">
    <div class="form_msg">
        <p>Are you sure to delete this Designation</p>
