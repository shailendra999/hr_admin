...
$gFinYear=$_REQUEST['finYear'];

if(($_REQUEST['byControl'])=='IndentNo'){
	$sql="select * from ms_indent_master mim,ms_indent_transaction mit where mim.indent_id=mit.indent_id and mim.indent_id=$val and mit.finYear = '".$gFinYear."' $Pending order by mim.indent_date asc";
}
else if(($_REQUEST['byControl'])=='Department'){
	$sql="select * from ms_indent_master mim,ms_indent_transaction mit where mim.indent_id=mit.indent_id and mim.department_id=$val and mit.finYear = '".$gFinYear."' $Pending order by mim.indent_date asc";
}
else if(($_REQUEST['byControl'])=='Item'){
	$sql="select * from ms_indent_master mim,ms_indent_transaction mit where mim.indent_id=mit.indent_id and mit.item_id=$val and mit.finYear = '".$gFinYear."' $Pending order by mim.indent_date asc";
}
else if(($_REQUEST['byControl'])=='IndentDate'){
	$val=getDateFormate($val);
	$sql="select * from ms_indent_master mim,ms_indent_transaction mit where mim.indent_id=mit.indent_id and mim.indent_date='".$val."' and mit.finYear = '".$gFinYear."' $Pending order by mim.indent_date asc";
}
else if(($_REQUEST['byControl'])=='isBackData'){
	$sql="select * from ms_indent_master mim, ms_indent_transaction mit where mim.indent_id = mit.indent_id $Pending order by mim.indent_date asc";
}
//echo $sql;
