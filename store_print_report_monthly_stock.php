...
<title>Print Monthly Stock Report</title>
<style>
.note
{
    ...
    <thead>
      <tr height="70px">
        <td align="center">
          ...
          STOCK CONSUMPTION REPORT BETWEEN <? echo '<b>Report From '.$start_from.' To '.$end.'</b>';?>
        </td>
      </tr>
    </thead>
    <tbody>
      <tr>
        <td>
        	<table align="center" width="100%" cellpadding="2" cellspacing="0" border="1" class="tblborder">  			
            <tr>
            <td align="center"></td>
            <?
                $start=$start_from;
                ...
                for ($i=1;$i<=$monthdiff;$i++) 
                {
                    ...
                    /* Total Issue Entry */
                        ...
                        $sql_p = "SELECT ms_IE_master.IE_id, sum( ms_IE_transaction.iss_qty ) AS Qty , ms_IE_transaction.item_id AS ItemId,
                                        ...
                                        ORDER BY `ms_IE_transaction`.`item_id` ASC";
          ...
          ?>
        	...
        	<?php /*?><table align="center" width="100%" cellpadding="0" cellspacing="0" border="1" class="tblborder">  			
            <tr>
              <td align="center"></td>
              <?
              $start=$start_from;
            ...
            <tr>
              ...
              for ($i=1;$i<=$monthdiff;$i++) 
              {
                ...
                $sql_os = "select
                  ...
                  (
                  select 
                  ifnull(sum(ms_IE_transaction.iss_qty*ifnull((select (ms_GRN_transaction.net_rate) from ms_GRN_transaction where ms_GRN_transaction.item_id = ms_IE_transaction.item_id),ifnull((select (ms_item_master.opening_rate) from ms_item_master where ms_item_master.item_id=ms_IE_transaction.item_id),0))),0)
                  ...
                  {
                    $row_os = mysql_fetch_array($result_os);
                    $OpeningQuantity=$row_os["OpeningQuantity"];
                  }
                  else	
                    $OpeningQuantity='0';
                  ?> 
                  <td align="right" class="padding_right"><?=number_format($OpeningQuantity,2,'.','');?></td>
                  <?
                  $m = date ("m-Y",mktime(0, 0, 0,$m+1, 1, $y));
                  $start=$m;
              }
            ...
            <tr>
              ...
              for ($i=1;$i<=$monthdiff;$i++) 
              {
                ...
                $sql_p = "select 
                  ...
                  {
                    $row_p = mysql_fetch_array($result_p);
                    $PurchaseQuantity=$row_p["PurchaseQuantity"];
                  }
                  else
                    $PurchaseQuantity='0';
                  ?>
                  <td align="right" class="padding_right"><?=number_format($PurchaseQuantity,2,'.','');?></td>
                  <?
                $m = date ("m-Y",mktime(0, 0, 0,$m+1, 1, $y));
                $start=$m;
              }
            ...
            <tr>
              ...
              for ($i=1;$i<=$monthdiff;$i++) 
              {
                ...
                $IssueQuantity=$row_p["IssueQuantity"];
                  ...
                  $IssueQuantity='0';
                ?>
                <td align="right" class="padding_right"><?=number_format($IssueQuantity,2,'.','');?></td>
                <?
                $m = date ("m-Y",mktime(0, 0, 0,$m+1, 1, $y));
                $start=$m;
              }
