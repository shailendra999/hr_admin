...
{
	$getLeave = 0;
	$sql = "SELECT count(*) as count FROM mpc_leave_application where emp_id = $emp_id and leave_type = '$leave_type'";
	//echo $sql;
...
{
	$getTakenLeave = "";
	$sql = "SELECT leave_type FROM mpc_leave_application where leave_date='$data_date' and emp_id='$emp_id'";
	//echo $sql;
	$result = mysql_query ($sql) or die ("Error in : ".$sql."<br>".mysql_errno()." : ".mysql_error());
...
function getCountryDetail($field_name,$id)
{
	$getCountryDetail = "";
	$sql = "SELECT ".$field_name." FROM esm_countries where countries_id = $id";
	//echo $sql;
	$result = mysql_query ($sql) or die ("Error in : ".$sql."<br>".mysql_errno()." : ".mysql_error());
	...
	if(mysql_num_rows($result)>0)
	{
		...
		$row = mysql_fetch_array($result);
		$getCountryDetail = $row[$field_name];
		
	}
	
	return $getCountryDetail;
}
function getStateDetail($field_name,$id)
{
	$getStateDetail = "";
	$sql = "SELECT ".$field_name." FROM esm_state_master where rec_id = $id";
	//echo $sql;
	$result = mysql_query ($sql) or die ("Error in : ".$sql."<br>".mysql_errno()." : ".mysql_error());
	...
	if(mysql_num_rows($result)>0)
	{
		...
		$row = mysql_fetch_array($result);
		$getStateDetail = $row[$field_name];
		
	}
	
	return $getStateDetail;
}
function getCityDetail($field_name,$id)
{
	$getCityDetail = "";
	$sql = "SELECT ".$field_name." FROM esm_city_master where rec_id = $id";
	//echo $sql;
	$result = mysql_query ($sql) or die ("Error in : ".$sql."<br>".mysql_errno()." : ".mysql_error());
...
{
	$getShiftDetail = "";
	$sql = "SELECT ".$field_name." FROM mpc_shift_detail where emp_id = '$emp_id'";
	//echo $sql;
	$result = mysql_query ($sql) or die ("Error in : ".$sql."<br>".mysql_errno()." : ".mysql_error());
...
?>
<?
function getadvance($emp_id)
{
	$getdesignationMaster="";
	 $sql = "SELECT sum(advance) as net_advance,sum(deduction) as net_deduction  FROM mpc_advance_employee where emp_id = '".$emp_id."'";
	$result = mysql_query ($sql) or die ("Error in : ".$sql."<br>".mysql_errno()." : ".mysql_error());
	if(mysql_num_rows($result)>0)
	{
		$row=mysql_fetch_array($result);
		
		$getdesignationMaster = $row['net_advance']-$row['net_deduction'];
	}
	return $getdesignationMaster;
}
?>
...
?>
<?
function getplantDetail($FieldName, $WhereFieldName, $WhereValue)
{
	$getplantDetail="";
	$sql = "SELECT ".$FieldName." FROM mpc_plant_master where ".$WhereFieldName." = '".$WhereValue."'";
	$result = mysql_query ($sql) or die ("Error in : ".$sql."<br>".mysql_errno()." : ".mysql_error());
	if(mysql_num_rows($result)>0)
	{
		$row=mysql_fetch_array($result);
...
?>
<?
function getunittDetail($FieldName, $WhereFieldName, $WhereValue)
{
	$getunittDetail="";
	$sql = "SELECT ".$FieldName." FROM ms_unit_master where ".$WhereFieldName." = '".$WhereValue."'";
	$result = mysql_query ($sql) or die ("Error in : ".$sql."<br>".mysql_errno()." : ".mysql_error());
	if(mysql_num_rows($result)>0)
	{
		$row=mysql_fetch_array($result);
		$getunittDetail = $row[$FieldName];
	}
	return $getunittDetail;
}
?>
<?
function getstock_item_Detail($FieldName, $WhereFieldName, $WhereValue)
{
	$getunittDetail="";
	$sql = "SELECT ".$FieldName." FROM ms_stock_master where ".$WhereFieldName." = '".$WhereValue."'";
	$result = mysql_query ($sql) or die ("Error in : ".$sql."<br>".mysql_errno()." : ".mysql_error());
	if(mysql_num_rows($result)>0)
	{
		$row=mysql_fetch_array($result);
...
?>
<?
function getStockOpeningQuantity($opening_date,$product_id,$company_id)
{
	$getStockOpeningQuantity="";
	$sql = "SELECT opening_stock from ms_purchase_company_product_master where product_id  = '".$product_id."' and company_id ='$company_id'";
	$result = mysql_query ($sql) or die ("Error in : ".$sql."<br>".mysql_errno()." : ".mysql_error());
	if(mysql_num_rows($result)>0)
	{
		while($row=mysql_fetch_array($result))
			{		
				$getStockOpeningQuantity =$row['opening_stock'];
			}
	}
	
	$sql = "select (SELECT sum(quantity) as quantity from ms_sell_master where toWhat  = '0' and company_id='$company_id' and sell_date < '$opening_date' and form_id !=0) as buy, (SELECT sum(quantity) as quantity from ms_buyer_master where fromWhat = '0' and company_id='$company_id' and buyer_date < '$opening_date' and form_id !=0) as sell";
	$result = mysql_query ($sql) or die ("Error in : ".$sql."<br>".mysql_errno()." : ".mysql_error());
	if(mysql_num_rows($result)>0)
	{
		$row=mysql_fetch_array($result);
		$getStockOpeningQuantity=$getStockOpeningQuantity+$row['buy']-$row['sell'];	
	}
	
	
	return $getStockOpeningQuantity;
}
?>
<?
	function getStockLedgerOpeningQuanityDepartment($department_id)
	{
		$getStockLedgerOpeningQuanityDepartment="";
		...
		$sql_sub="SELECT 
						...
						WHERE
						 ...
						 and ms_item_master.department_id='".$department_id."'
	...
	return $getStockLedgerOpeningQuanityDepartment;	
	}
?>
