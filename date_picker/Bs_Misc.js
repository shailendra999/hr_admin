...
function getAbsolutePos(el, stopIfAbsolute) {
	...
	/*
	//new code but not correct somehow
	if (!el) return false;
	var x=0, y=0;
	while (el != null) {
		...
		if (stopIfAbsolute == true) {
			if (el.offsetParent.currentStyle && el.offsetParent.currentStyle.position) {
				...
				if (position == 'relative') break; //hrm, not sure why i added this...
				var overflow = el.offsetParent.currentStyle.overflow;
...
function encodeFilename(filename, e) {
	...
	*/
	...
	/*
	//NEW, BETTER CODE
	//todo: causes problems with different charsets (eg if the page itself that includes this is defined as utf8, in ie, not fox.)
	//the web server would have to specify that the js is utf8 too, for example using .htaccess "AddCharset utf-8 .js"
