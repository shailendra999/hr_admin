...
$Page = "store_add_purchase_indent.php";
...
if(isset($_GET['mode'])){
	...
	{
		echo "<script>alert('You Cant Edit, Purchase Order Or GRN is Made for this Indent.');location.href='store_list_purchase_indent.php';</script>";
	}
}
if(isset($_GET[$PageKey])){
	...
	if(mysql_num_rows($result)>0){
		...
		$PageKeyValue = $row[$PageKey];$indent_id = $row[$PageKey]; $indent_number= $row['indent_number'];
...
#	$indent_number = $_POST['indent_id'];
	...
	if($PageKeyValue == ""){print_r($_POST);
		$tableName="ms_indent_master";
		$tableData=array("''","'$indent_number'","'$indent_date'","'$department_id'","'$approved_by'","now()","'$gFinYear'");
		if(addDataIntoTable($tableName,$tableData))
		{
			...
			for($i=0; $i<sizeof($_POST['sno']); $i++)
			{
				if($_POST['item_id'][$i]!='' && $_POST['req_qty'][$i]!="")
				{
					...
					//print_r($tableData);
					$tableData=array("''","'$insert_id'","'$item_id'","'$required_quantity'","'$pend_qty'","'$due_date'","'$remark'","'$purpose'","now()","'$gFinYear'");
		...
		if($mode == "edit"){
			$tableName="ms_indent_master";
			$tableColumns=array("indent_id","indent_number","indent_date","department_id","approved_by");
			...
			if(updateDataIntoTable($tableName,$tableColumns,$tableData)){
				...
				for($i=0; $i<sizeof($_POST['sno']); $i++){	
					if($_POST['item_id'][$i]!='' && $_POST['req_qty'][$i]!=""){
						...
						if($indent_transaction_id == ""){
								$tableName="ms_indent_transaction";
								$tableData=array("''","'$PageKeyValue'","'$item_id'","'$required_quantity'","'$pend_qty'","'$due_date'","'$remark'","'$purpose'","now()","'$gFinYear'");
								//print_r($tableData);
								addDataIntoTable($tableName,$tableData);
						}else{
				...
				//echo $Message;
				redirect("store_list_purchase_indent.php?Message=$Message");
			}
		}
	}
}
...
if(isset($_GET["indent_id"])){
	...
	$sql="select max(indent_id) as indent_id, max(indent_number) as indent_number from ms_indent_master";
	$res=mysql_query($sql);
	$row=mysql_fetch_array($res);
	$indent_id=($row['indent_id']+1);
	
	$sqlf="select finYear from ms_indent_master where indent_id = '".$row['indent_id']."'";
	$resf=mysql_query($sqlf);
	$rowf=mysql_fetch_array($resf);
	if($rowf['finYear'] != $gFinYear){
		$indent_number =1;
	}else{
		$sqlfy="select max(indent_number) as indent_number from ms_indent_master where finYear = '".$gFinYear."'";
		$resfy=mysql_query($sqlfy);
		$rowfy=mysql_fetch_array($resfy);
		$indent_number=($rowfy['indent_number']+1);
	}
//	$indent_id = ($row['indent_number']=="") ? $indent_id : $indent_number;
}
?>
                        ...
                        <td align="center" valign="top" class="border" width="100%" bgcolor="#EAE3E1">
                          <table width="100%" cellpadding="2" cellspacing="2" align="center" border="0" class="border text_1">
                            <tr>
                            	...
                            	<td align="left">
                              	<input type="text" name="indent_number" id="indent_number" readonly="readonly" value="<?=$indent_number?>" />
