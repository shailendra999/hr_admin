...
if (isset($_POST["btn_del"])) {
    ...
    $sql = "delete from mpc_department_employee where rec_id = '" . $rec_id . "'";
    mysql_query($sql) or die("Invalid query : " . $sql . "<br>" . mysql_errno() . " : " . mysql_error());
    $msg = "Department Entry Sucessfully Deleted";
}
...
if (isset($_POST['department'])) {
    if ($_POST['no_refresh'] == $_SESSION['no_refresh']) {
        ...
        $sql = "SELECT * FROM " . $mysql_table_prefix . "employee_master where ticket_no=$id";
        $result1 = mysql_query($sql) or die("Error in : " . $sql . "<br>" . mysql_errno() . " : " . mysql_error());
        $row = mysql_fetch_array($result1);
        $rec_id = $row['emp_id'];
        $sql = "SELECT * FROM " . $mysql_table_prefix . "department_employee where emp_id = '$rec_id' and ((
		'$txt_from_date'>=from_date and '$txt_from_date'<=to_date) or ('$txt_to_date'>=to_date))and rec_id !='$dept_id'";
            ...
            while ($row = mysql_fetch_array($result)) {
                ...
                if (strtotime($txt_from_date) >= strtotime($row['from_date']) and strtotime($txt_from_date) <= strtotime($row['to_date'])) {
                    ...
                    $sql_check = "Update " . $mysql_table_prefix . "department_employee set	
																						  to_date ='$txt_from_date',dept_id='$department' where emp_id='$rec_id'";
                    ...
                    $sql_check = "Update " . $mysql_table_prefix . "department_employee set	
																						   from_date ='$txt_to_date',dept_id='$department' where emp_id='$rec_id'";
                    ...
                    $sql = "delete from " . $mysql_table_prefix . "department_employee where emp_id='$rec_id' and to_date!='0000-00-00'";
                    mysql_query($sql) or die("Invalid query : " . $sql . "<br>" . mysql_errno() . " : " . mysql_error());
        ...
        $sql_check = "Update " . $mysql_table_prefix . "department_employee set	dept_id ='$department',from_date='$txt_from_date',to_date ='$txt_to_date' where emp_id='$rec_id'";
        ...
        $sql_check = "Update mpc_employee_master set sub_department='$department',department='$sub_department' where 'ticket_no='$id'";
    ...
    <tr>
        ...
        <td style="padding-left:5px; padding-top:5px;" valign="top">
            <table width="100%" cellpadding="0" cellspacing="0" align="center" border="0">
                <tr>
                    <td align="center" class="gray_bg"><img src="images/bullet.gif" width="15" height="22"/> &nbsp; Employee Department</td>
                </tr>
                <tr>
            ...
            get_frm_menu('employee_list_department.php', this.value, 'emp_list', '');
        }" value="<?= $ticket_no ?>"/>
                                            </td>
                                            <td width="24%" class="text_1" style="padding-top:5px;">
                                                <form name="frm_check" id="frm_check">
                                                    <input type="radio" id="search_by" name="search_by" value="ID" checked="checked" onclick="get_frm_menu('employee_list_department.php', document.getElementById('emp_search').value, 'emp_list', '')" />ID
                                                    <input type="radio" id="search_by" name="search_by"  value="Name" onclick="get_frm_menu('employee_list_department.php', document.getElementById('emp_search').value, 'emp_list', '')"/>Department Name
                                                    <input type="radio" id="search_by" name="search_by"  value="Employee" onclick="get_frm_menu('employee_list_department.php', document.getElementById('emp_search').value, 'emp_list', '')"/>Employee Name
                                                </form>  
                                            </td>
                                        </tr>
                                    </table>
                            ...
                            <tr>
                                <td valign="top">
                                    <div id="emp_list" style="overflow:scroll;height:300px;">
                                        <?php include('load/load_department_first.php') ?>
                                    </div>
                                </td>
                            </tr>
                        </table>
