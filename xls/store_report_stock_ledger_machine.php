<? 
	...
	function OpeningValue($ItemId, $LeftQuantity){
		...
		if(mysql_num_rows($res_purchase)>0){
			while($row_purchase=mysql_fetch_array($res_purchase)){
				if($LeftQuantity > $row_purchase["rec_qty"] && $GRNLeftQuantity<=0){$PurchaseValue += $row_purchase["rec_qty"] * $row_purchase["net_rate"];
	...
	$html_string.='<caption>'.$company_Fulladdress.'<br /><strong>Stock Ledger Machine Report '.$DateRange.'</strong></caption>';
	$html_string.='<tr><td><strong>S.No.</strong></td><td><strong>Machinery</strong></td><td><strong>Opening Value</strong></td><td><strong>Purchase Value</strong></td><td><strong>Issue Value</strong></td><td><strong>Closing Value</strong></td></tr>';
	
	
$tItemId = 3438;
$sql_department="SELECT machinary_id as department_id, name from ms_machinary where machinary_id in (select machinary_id from ms_item_master) ORDER BY name";
...
if(mysql_num_rows($result_department)>0){
	...
	while($row_department = mysql_fetch_array($result_department)){
		...
		from 
			ms_item_master 
		where 
			ms_item_master.machinary_id	='".$row_department['department_id']."'";
		$TotalIValue = 0;	
		$OpeningValue = 0;
		$IssueValue = 0;
		$OpneingIssueValue = 0;
		...
		if(mysql_num_rows($res_opening_value)>0){
			while($row_opening_value=mysql_fetch_array($res_opening_value)){
				...
				where 
					...
					and ms_item_master.machinary_id='".$row_department['department_id']."'
		...
		if(mysql_num_rows($result_purchase)>0){
			while($row_purchase = mysql_fetch_array($result_purchase)){
				...
				$html_string.='<tr><td>'.$sno++.'</td><td>'.$row_department['name'].'</td><td>'.number_format($OpeningValue1,2).'</td><td>'.number_format($TotalPurchase,2).'</td><td>'.number_format($IssueValue,2).'</td><td>'.number_format($OpeningValue1+$TotalPurchase-$IssueValue,2).'</td></tr>';
...
$html_string.='<tr><td></td><td><strong>Total</strong></td><td>'.number_format($TotalOpeningValue,2).'</td><td>'.number_format($TotalPurchaseValue,2).'</td><td>'.number_format($TotalIssueValue,2).'</td><td>'.number_format($TotalClosingValue,2).'</td></tr>';

	$html_string.='</table>';
	//make time dependent xls name that is always unique
	$xlsfile = "Store_Report_Stock_Ledger_Machine".date("m-d-Y-hiA").".xls";
